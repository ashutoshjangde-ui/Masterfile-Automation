...
# Valid values: Granulated, Liquid, Liquid Concentrate, Powdered
    ...
    for c in ordered_cols:
        ...
        for label, pats in _FD_PATTERNS.items():
            ...
            if label == "Liquid" and is_non_food_liquid:
                score *= 0.1  # strongly demote non-food liquid contexts
            scores[label] += score

    # If Liquid Concentrate is detected, suppress plain Liquid so LC wins
...
if go:
    ...
    try:
        ...
        # Step 3.7: infer Gender, H&B Subtype, Food & Drink Form 1 from SEO columns
        try:
